/* mnhb.f -- translated by f2c (version 20100827).
...
/* Subroutine */ int mnhb_(integer *n, real *d__, real *x, real *b, S_fp 
	calcf, S_fp calcgh, integer *iv, integer *liv, integer *lv, real *v, 
	integer *uiparm, real *urparm, U_fp ufparm)
{
    ...
    extern /* Subroutine */ int rmnhb_(real *, real *, real *, real *, real *,
...
/*        THIS ROUTINE IS LIKE   MNGB, EXCEPT THAT THE SUBROUTINE PARA- */
/*     METER CALCG OF   MNGB (WHICH COMPUTES THE GRADIENT OF THE OBJEC- */
...
/*     AS FOR   MNGB, WHILE H IS AN ARRAY OF LENGTH N*(N+1)/2 IN WHICH */
...
/*             INITIALIZED BY CALLING   MNHB WITH IV(1) = 13.) */
...
/* IVSET.... PROVIDES DEFAULT INPUT VALUES FOR IV AND V. */
/*  RMNHB... REVERSE-COMMUNICATION ROUTINE THAT DOES   MNHB ALGORITHM. */
...
/* +++++++++++++++++++++++++++++++  BODY  ++++++++++++++++++++++++++++++++ */
    ...
    rmnhb_(&b[3], &d__[1], &f, &v[1], &v[1], &iv[1], &lh, liv, lv, n, &v[1], &
	    x[1]);
    if (iv[1] != 14) {
	goto L999;
    }
...
L20:
    rmnhb_(&b[3], &d__[1], &f, &v[g1], &v[h1], &iv[1], &lh, liv, lv, n, &v[1],
	     &x[1]);
    if ((i__1 = iv[1] - 2) < 0) {
	goto L30;
    } else if (i__1 == 0) {
