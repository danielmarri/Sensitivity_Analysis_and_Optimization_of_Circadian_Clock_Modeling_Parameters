/* l7svx.f -- translated by f2c (version 20100827).
...
doublereal l7svx_(integer *p, real *l, real *x, real *y)
{
...
/*       ***  THAT X(I) HOLDS THE CURRENT PARTIAL SUM FOR ROW I. */
	...
	for (i__ = 1; i__ <= i__2; ++i__) {
	    ji = j0 + i__;
	    blji = b * l[ji];
	    splus += (r__1 = blji + x[i__], dabs(r__1));
	    sminus += (r__1 = blji - x[i__], dabs(r__1));
/* L20: */
	}
	if (sminus > splus) {
	    b = -b;
...
/*  ***  NORMALIZE Y AND SET X = (L**T)*Y  *** */
	...
	yi = t * y[i__];
	x[i__] = 0.f;
	v2axy_(&i__, &x[1], &yi, &l[ji], &x[1]);
	ji += i__;
/* L70: */
    }
    ret_val = v2nrm_(p, &x[1]);
    goto L999;
