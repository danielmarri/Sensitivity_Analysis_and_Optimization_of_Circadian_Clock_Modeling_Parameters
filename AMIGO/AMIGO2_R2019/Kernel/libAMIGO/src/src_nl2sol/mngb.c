/* mngb.f -- translated by f2c (version 20100827).
...
/* Subroutine */ int mngb_(integer *n, real *d__, real *x, real *b, S_fp 
	calcf, S_fp calcg, integer *iv, integer *liv, integer *lv, real *v, 
...
/*        THIS ROUTINE IS LIKE   MNG, EXCEPT FOR THE EXTRA PARAMETER B, */
/*     AN ARRAY OF LOWER AND UPPER BOUNDS ON X...   MNGB ENFORCES THE */
/*     CONSTRAINTS THAT  B(1,I) .LE. X(I) .LE. B(2,I), I = 1(1)N. */
/*     (INSTEAD OF CALLING  RMNG,   MNGB CALLS  RMNGB.) */
/* . */
...
/*  RMNGB... REVERSE-COMMUNICATION ROUTINE THAT CARRIES OUT   MNG ALGO- */
/*             RITHM. */
...
/* +++++++++++++++++++++++++++++++  BODY  ++++++++++++++++++++++++++++++++ */
    ...
    rmngb_(&b[3], &d__[1], &f, &v[1], &iv[1], liv, lv, n, &v[1], &x[1]);
    if (iv[1] != 14) {
	goto L999;
    }
...
L20:
    rmngb_(&b[3], &d__[1], &f, &v[g1], &iv[1], liv, lv, n, &v[1], &x[1]);
    if ((i__1 = iv[1] - 2) < 0) {
	goto L30;
    } else if (i__1 == 0) {
	goto L40;
