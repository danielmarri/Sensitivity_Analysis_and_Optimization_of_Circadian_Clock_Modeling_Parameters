/* dn2pb.f -- translated by f2c (version 20100827).
...
/* Subroutine */ int dn2pb_(integer *n, integer *nd, integer *p, doublereal *
	x, doublereal *b, S_fp calcr, S_fp calcj, integer *iv, integer *liv, 
	integer *lv, doublereal *v, integer *ui, doublereal *ur, U_fp uf)
{
...
/* /6 */
/*     INTEGER IV(LIV), UI(1) */
/*     DOUBLE PRECISION B(2,P), X(P), V(LV), UR(1) */
/* /7 */
/* / */
...
/* ---------------------------------  BODY  ------------------------------ */
    ...
    drn2gb_(&b[3], &v[1], &v[1], &iv[1], liv, lv, n, &nd1, &n1, &n2, p, &v[1],
	     &v[1], &v[1], &x[1]);
    if (iv[1] != 14) {
	goto L999;
    }
...
L20:
    drn2gb_(&b[3], &v[d1], &v[dr1], &iv[1], liv, lv, n, &nd1, &n1, &n2, p, &v[
	    r1], &v[rd1], &v[1], &x[1]);
    iv1 = iv[1];
    if ((i__1 = iv1 - 2) < 0) {
	goto L40;
