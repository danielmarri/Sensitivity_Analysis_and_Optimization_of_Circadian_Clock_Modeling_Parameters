/* rn2g.f -- translated by f2c (version 20100827).
...
/* Subroutine */ int rn2g_(real *d__, real *dr, integer *iv, integer *liv, 
	integer *lv, integer *n, integer *nd, integer *n1, integer *n2, 
	integer *p, real *r__, real *rd, real *v, real *x)
{
    ...
    static real t;
    static integer g1, y1, gi, lh, nn, yi, iv1, qtr1;
    extern /* Subroutine */ int q7rad_(integer *, integer *, integer *, real *
	    ...
	    d7upd_(real *, real *, integer *, integer *, integer *, integer *,
	     integer *, integer *, integer *, integer *, real *), q7apl_(
	    integer *, integer *, integer *, real *, real *, integer *), 
	    g7lit_(real *, real *, integer *, integer *, integer *, integer *,
	     integer *, real *, real *, real *);
    static integer rmat1;
    extern /* Subroutine */ int n2cvp_(integer *, integer *, integer *, 
	    integer *, real *);
    static integer jtol1;
    ...
    extern /* Subroutine */ int v7cpy_(integer *, real *, real *), ivset_(
...
/*        IV(RDREQ) IS NONZERO.    RN2G SETS IV(REGD) = 1 IF RD */
...
/*  WHEN   RN2G IS CALLED WITH IV(1) = 0 OR 12.    RN2G ALSO ALLOWS */
/*  R AND DR TO BE SUPPLIED ROW-WISE -- JUST SET ND = 1 AND CALL */
/*    RN2G ONCE FOR EACH ROW WHEN PROVIDING RESIDUALS AND JACOBIANS. */
/*     ANOTHER NEW FEATURE IS THAT CALLING   RN2G WITH IV(1) = 13 */
...
/*  AND IV(1) WILL HAVE BEEN SET TO 14. CALLING   RN2G WITH IV(1) = 14 */
/*  CAUSES EXECUTION OF THE ALGORITHM TO BEGIN UNDER THE ASSUMPTION */
/*  THAT STORAGE HAS BEEN ALLOCATED. */

/* ***  SUPPLYING R AND DR  *** */

/*       RN2G USES IV AND V IN THE SAME WAY AS NL2SOL, WITH A SMALL */
/*  NUMBER OF OBVIOUS CHANGES.  ONE DIFFERENCE BETWEEN   RN2G AND */
/*  NL2ITR IS THAT INITIAL FUNCTION AND GRADIENT INFORMATION NEED NOT */
/*  BE SUPPLIED IN THE VERY FIRST CALL ON   RN2G, THE ONE WITH */
/*  IV(1) = 0 OR 12.  ANOTHER DIFFERENCE IS THAT   RN2G RETURNS WITH */
...
/*     ANOTHER NEW RETURN...   RN2G IV(1) = -1 WHEN IT WANTS BOTH THE */
/*  RESIDUAL AND THE JACOBIAN TO BE EVALUATED AT X. */
/*     A NEW RESIDUAL VECTOR MUST BE SUPPLIED WHEN   RN2G RETURNS WITH */
...
/*  REQUIRED, I.E., WHEN   RN2G RETURNS WITH IV(1) = 2, -1, OR -2. */
/*  NOTE THAT   RN2G OVERWRITES R, BUT THAT IN THE SPECIAL CASE OF */
/*  N1 = 1 AND N2 = N ON PREVIOUS CALLS,   RN2G NEVER RETURNS WITH */
...
/*     SEE THE COMMENTS IN SUBROUTINE    N2G. */
...
/* G7LIT.... PERFORMS BASIC MINIMIZATION ALGORITHM. */
...
/*  N2LRD... COMPUTES REGRESSION DIAGNOSTICS. */
...
L10:
    ...
    iv[4] += *p * (*p + 13) / 2;
L20:
    g7lit_(&d__[1], &x[1], &iv[1], liv, lv, p, p, &v[1], &x[1], &x[1]);
    if (iv[1] != 14) {
	goto L999;
    }
...
/*  ***  -- ASK FOR BOTH RESIDUAL AND JACOBIAN AT ONCE */
    ...
    g1 = iv[28];
    y1 = iv[48];
    g7lit_(&d__[1], &v[g1], &iv[1], liv, lv, p, p, &v[1], &x[1], &v[y1]);
    if (iv[1] != 1) {
	goto L220;
    }
    v[10] = 0.f;
...
L240:
    l = iv[42];
    n2lrd_(&dr[dr_offset], &iv[1], &v[l], &lh, liv, lv, nd, &nn, p, &r__[1], &
	    rd[1], &v[1]);
    if (*n2 < *n) {
	goto L270;
    }
...
/*     ***  USE STEP VECTOR (ALLOCATED BY G7LIT) FOR SCRATCH. */

    rmat1 = iv[78];
    v7scp_(&lh, &v[rmat1], &c_b14);
    q7rad_(&nn, nd, p, &r__[1], &c_false, &v[rmat1], &dr[dr_offset], &r__[1]);
    iv[69] = 0;
...
L250:
    l = iv[42];
    c7vfn_(&iv[1], &v[l], &lh, liv, lv, n, p, &v[1]);
    goto L290;
...
L300:
    ...
    if (iv[1] <= 6 && iv[57] > 0) {
	n2cvp_(&iv[1], liv, lv, p, &v[1]);
    }

L999:
    return 0;
/*  ***  LAST LINE OF   RN2G FOLLOWS  *** */
} /* rn2g_ */
