/* a7sst.f -- translated by f2c (version 20100827).
...
/* Subroutine */ int a7sst_(integer *iv, integer *liv, integer *lv, real *v)
{
    ...
    static real gts, emax, xmax, rfac1, emaxs;
...
/*             UNCHANGED SINCE THE PREVIOUS RETURN OF A7SST. */
...
/*             IN WHICH CASE A7SST SETS IV(SWITCH) = 1. */
...
/*             THAN V(AFCTOL) AND A7SST DOES NOT RETURN WITH */
/*             IV(IRC) = 11, THEN A7SST RETURNS WITH IV(IRC) = 10. */
...
/*             V(PREDUC) .LE. V(SCTOL) * ABS(V(F0)), THEN A7SST RETURNS */
/*             WITH IV(IRC) = 11.  IF SO DOING APPEARS WORTHWHILE, THEN */
/*             A7SST REPEATS THIS TEST (DISALLOWING A FULL NEWTON STEP) */
...
/*             NEWTON STEP.  IF A7SST IS CALLED WITH IV(IRC) = 6, I.E., */
...
/*             A7SST RETURNS WITH IV(IRC) = 8 OR 9. */
...
/*     A7SST IS DESIGNED TO HANDLE ANY NUMBER OF MODELS. */
...
/*     CHANGE THE STOPPING TOLERANCES AND CALL A7SST AGAIN, IN WHICH */
...
L160:
    ...
    if (emax < v[4]) {
/* Computing MAX */
	r__1 = v[24], r__2 = v[4] * .5f / emax;
	v[16] = rfac1 * dmax(r__1,r__2);
    }
...
/*        ***  STEP. */
    ...
    v[16] = v[25];
    gts = v[4];
    if (v[11] < (.5f / v[16] - 1.f) * gts) {
/* Computing MAX */
	r__1 = v[23], r__2 = gts * .5f / (gts + v[11]);
	v[16] = dmax(r__1,r__2);
    }
    iv[29] = 4;
    if (v[5] == 0.f) {
    ...
    if (v[3] >= 0.f && (v[3] < v[2] * 2.f || v[6] < v[11] * 1.2f)) {
	goto L230;
    }
...
L240:
    ...
    i__ = 0;
    if (v[6] > 0.f && v[6] <= emax || v[6] == 0.f && v[7] == 0.f) {
	i__ = 2;
    }
    if (v[5] == 0.f && v[17] <= v[33] && goodx) {
	++i__;
    }
    if (i__ > 0) {
	iv[29] = i__ + 6;
