function stop = gcv_plotfcn(x,optimValues,state,varargin)
...
%   $Revision: 1.1.6.5 $  $Date: 2010/09/02 13:35:50 $
    ...
    case 'iter'
        if isfield(optimValues,'fval')
            if isscalar(optimValues.fval)
                plotscalar(optimValues.iteration,x,optimValues.fval);
            else
...
function plotscalar(iteration,x,fval)
% PLOTSCALAR initializes or updates a line plot of the function value
% at each iteration.

if iteration == 0
    plotfval = plot(x,fval,'kd','MarkerFaceColor',[1 0 1]);
