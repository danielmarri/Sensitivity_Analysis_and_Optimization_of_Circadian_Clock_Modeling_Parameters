/* dmng.f -- translated by f2c (version 20100827).
...
/* Subroutine */ int dmng_(integer *n, doublereal *d__, doublereal *x, S_fp 
...
/*                  D CAN STRONGLY AFFECT THE BEHAVIOR OF  DMNG. */
...
/*                  THE DEFAULTS PROVIDED BY SUBROUTINE DIVSET (SEE IV */
/*                  BELOW) REQUIRE THE CALLER TO SUPPLY D. */
/* X........ (INPUT/OUTPUT) BEFORE (INITIALLY) CALLING  DMNG, THE CALL- */
/*                  ER SHOULD SET  X  TO AN INITIAL GUESS AT  X*.  WHEN */
/*                   DMNG RETURNS,  X  CONTAINS THE BEST POINT SO FAR */
...
/*                  0.  IN THIS CASE,  DMNG WILL RETURN WITH IV(1) = 65. */
...
/*                  BELOW) THAT HELPS CONTROL THE  DMNG ALGORITHM AND */
...
/*                  IS TOO SMALL, THEN  DMNG RETURNS WITH IV(1) = 15. */
/*                  WHEN  DMNG RETURNS, THE SMALLEST ALLOWED VALUE OF */
/*                  LIV IS STORED IN IV(LASTIV) -- SEE THE SECTION ON */
/*                  IV OUTPUT VALUES BELOW.  (THIS IS INTENDED FOR USE */
/*                  WITH EXTENSIONS OF  DMNG THAT HANDLE CONSTRAINTS.) */
/* LV....... (INPUT) LENGTH OF V ARRAY.  MUST BE AT LEAST 71+N*(N+15)/2. */
/*                  (AT LEAST 77+N*(N+17)/2 FOR  DMNF, AT LEAST */
/*                  78+N*(N+12) FOR  DMNH).  IF LV IS TOO SMALL, THEN */
/*                   DMNG RETURNS WITH IV(1) = 16.  WHEN  DMNG RETURNS, */
...
/*                  (SEE BELOW) THAT HELPS CONTROL THE  DMNG ALGORITHM */
...
/*             BE INITIALIZED BY CALLING  DMNG WITH IV(1) = 13 (SEE */
...
/*             FICE, THEN  DMNG RETURNS WITH IV(1) = 9.  DEFAULT = 200. */
...
/*             ITERATIONS DO NOT SUFFICE, THEN  DMNG RETURNS WITH */
/*             IV(1) = 10.  DEFAULT = 150. */
/* IV(OUTLEV)... IV(19) CONTROLS THE NUMBER AND LENGTH OF ITERATION SUM- */
/*             MARY LINES PRINTED (BY DITSUM).  IV(OUTLEV) = 0 MEANS DO */
...
/*                  ABOVE). */
/*            67 = BAD FIRST PARAMETER TO DIVSET. */
...
/*  ***  (SELECTED) V INPUT VALUES (FROM SUBROUTINE DIVSET)  *** */
...
/*             IF  DMNG FINDS A POINT WHERE THE FUNCTION VALUE IS LESS */
/*             THAN V(AFCTOL) IN ABSOLUTE VALUE, AND IF  DMNG DOES NOT */
...
/*             ITERATION, AND IF  DMNG DOES NOT RETURN WITH IV(1) = 3, */
...
/*             DECREASE, THEN  DMNG RETURNS WITH IV(1) = 4 (OR 5). */
...
/*             TION DECREASE, THEN  DMNG RETURNS WITH IV(1) = 3 (OR 5). */
...
/*     AND V OTHER THAN THE INPUT VALUES (THOSE SUPPLIED BY DIVSET). */
/*        THOSE WHO DO NOT WISH TO WRITE A CALCG WHICH COMPUTES THE */
/*     GRADIENT ANALYTICALLY SHOULD CALL  DMNF RATHER THAN  DMNG. */
/*      DMNF USES FINITE DIFFERENCES TO COMPUTE AN APPROXIMATE GRADIENT. */
...
/*     TINES CALCF AND CALCG MAY CALL ON DRMNG DIRECTLY.  SEE THE COM- */
/*     MENTS AT THE BEGINNING OF DRMNG. */
/*        THOSE WHO USE  DMNG INTERACTIVELY MAY WISH TO SUPPLY THEIR */
/*     OWN STOPX FUNCTION, WHICH SHOULD RETURN .TRUE. IF THE BREAK KEY */
/*     HAS BEEN PRESSED SINCE STOPX WAS LAST INVOKED.  THIS MAKES IT */
/*     POSSIBLE TO EXTERNALLY INTERRUPT  DMNG (WHICH WILL RETURN WITH */
...
/*        THE  DMNG DISTRIBUTION TAPE CONTAINS BOTH SINGLE- AND DOUBLE- */
/*     PRECISION VERSIONS OF THE  DMNG SOURCE CODE, SO IT SHOULD BE UN- */
/*     NECESSARY TO CHANGE PRECISIONS. */
/*        ONLY THE FUNCTIONS I7MDCN AND DR7MDC CONTAIN MACHINE-DEPENDENT */
...
/*        THE  DMNG SOURCE CODE IS EXPRESSED IN 1966 ANSI STANDARD */
...
/*     APPEAR ONLY IN MODULES DITSUM AND DPARCK.  PARAMETER STATEMENTS */
/*     APPEAR NEARLY EVERYWHERE.)  THESE CHANGES ALSO ADD SAVE STATE- */
/*     MENTS FOR VARIABLES GIVEN MACHINE-DEPENDENT CONSTANTS BY DR7MDC. */
...
/* DRMNG... REVERSE-COMMUNICATION ROUTINE THAT CARRIES OUT  DMNG ALGO- */
/*             RITHM. */
...
L20:
    drmng_(&d__[1], &f, &v[g1], &iv[1], liv, lv, n, &v[1], &x[1]);
    if ((i__1 = iv[1] - 2) < 0) {
	goto L30;
    } else if (i__1 == 0) {
	goto L40;
