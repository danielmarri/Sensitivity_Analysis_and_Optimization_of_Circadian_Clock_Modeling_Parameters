/* l7mst.f -- translated by f2c (version 20100827).
...
/* Subroutine */ int l7mst_(real *d__, real *g, integer *ierr, integer *
	ipivot, integer *ka, integer *p, real *qtr, real *r__, real *step, 
	real *v, real *w)
{
    /* Initialized data */
    ...
    static real lk, si, sj, uk, wl;
    static integer lk0, ip1, uk0;
    static real adi, rad, phi;
    static integer res;
    static real dst;
    ...
    extern /* Subroutine */ int l7itv_(integer *, real *, real *, real *), 
	    v7cpy_(integer *, real *, real *);
    extern doublereal l7svn_(integer *, real *, real *, real *);
    static integer kalim;
    static real alphak, dfacsq, psifac, oldphi, phimin, phimax;
    static integer phipin, dstsav;
    static real sqrtak, twopsi;
...
/*   IERR (I/O) = RETURN CODE FROM QRFACT OR Q7RGS -- 0 MEANS R HAS */
/*             FULL RANK. */
/* IPIVOT (I/O) = PERMUTATION ARRAY FROM QRFACT OR Q7RGS, WHICH COMPUTE */
/*             QR DECOMPOSITIONS WITH COLUMN PIVOTING. */
/*     KA (I/O).  KA .LT. 0 ON INPUT MEANS THIS IS THE FIRST CALL ON */
/*              L7MST FOR THE CURRENT R AND QTR.  ON OUTPUT KA CON- */
...
/*  L7ITV - APPLY INVERSE-TRANSPOSE OF COMPACT LOWER TRIANG. MATRIX. */
/*  L7IVM - APPLY INVERSE OF COMPACT LOWER TRIANG. MATRIX. */
...
/* Computing 2nd power */
	r__1 = rad;
	psifac = v[19] / (((v[20] + 1.f) * 8.f + 3.f) * (r__1 * r__1));
    }
    if (big <= 0.f) {
	big = r7mdc_(&c__6);
    }
...
L20:
    ...
    t = l7svn_(p, &r__[1], &step[1], &w[res]);
    if (t >= 1.f) {
	goto L30;
    }
    if (v2nrm_(p, &qtr[1]) >= big * t) {
	goto L90;
    }
...
/*     ***  TREAT IT AS SUCH WHEN USING  L7ITV AND  L7IVM. */
L30:
    l7itv_(p, &w[1], &r__[1], &qtr[1]);
...
/* L70: */
    ...
    l7ivm_(p, &step[1], &r__[1], &step[1]);
    t = 1.f / v2nrm_(p, &step[1]);
    w[phipin] = t / rad * t;
    lk = phi * w[phipin];
...
L110:
    ++(*ka);
    v7cpy_(&pp1o2, &w[rmat], &r__[1]);
    v7cpy_(p, &w[res], &qtr[1]);

/*  ***  SAFEGUARD ALPHAK AND INITIALIZE FAST GIVENS SCALE VECTOR.  *** */

    if (alphak <= 0.f || alphak < lk || alphak >= uk) {
/* Computing MAX */
...
/*  ***  CHECK FOR (AND HANDLE) SPECIAL CASE  *** */
    ...
    twopsi = alphak * dst * dst - d7tpr_(p, &step[1], &g[1]);
...
L440:
/* Computing MIN */
    r__1 = d7tpr_(p, &step[1], &g[1]);
    v[4] = dmin(r__1,0.f);
    v[7] = (alphak * dst * dst - v[4]) * .5f;
L450:
    v[2] = dst;
    w[dstsav] = dst;
    w[lk0] = lk;
