/* nsg.f -- translated by f2c (version 20100827).
...
/* Subroutine */ int nsg_(integer *n, integer *p, integer *l, real *alf, real 
	*c__, real *y, S_fp calca, S_fp calcb, integer *inc, integer *iinc, 
	...
	real *urparm, U_fp ufparm)
{
    ...
    extern /* Subroutine */ int rnsg_(real *, real *, real *, real *, integer 
	    *, integer *, integer *, integer *, integer *, integer *, integer 
	    *, integer *, integer *, integer *, real *, real *), ivset_(
...
/*     REAL ALF(P), C(L), URPARM(1), V(LV), Y(N) */
/* /7 */
/* / */
...
/* T(1)...T(N),    NSG ATTEMPTS TO COMPUTE A LEAST SQUARES FIT */
...
/*                      OF IV AND V BEFORE CALLING    NSG. */

/* LIV     INTEGER      LENGTH OF IV.  MUST BE AT LEAST  115+P+L + 2*M, */
/*                      WHERE  M  IS THE NUMBER OF ONES IN INC. */

/* LV      INTEGER      LENGTH OF V.  MUST BE AT LEAST */
/*                      105 + N*(L+M+3) + JLEN + L*(L+3)/2 + P*(2*P+17), */
/*                      WHERE  M  IS AS FOR LIV (SEE ABOVE) AND */
/*                      JLEN = (L+P)*(N+L+P+1),  UNLESS NEITHER A */
/*                      COVARIANCE MATRIX NOR REGRESSION DIAGNOSTICS ARE */
/*                      REQUESTED, IN WHICH CASE  JLEN = N*P.  IF THE */
...
/*                      THE PORT DOCUMENTATION FOR A COMPLETE LIST.  IF */
/*                      A COVARIANCE ESTIMATE IS REQUESTED, IT IS FOR */
...
/*   RNSG... CARRIES OUT NL2SOL ALGORITHM. */
...
L60:
    rnsg_(&v[1], &alf[1], &c__[1], &v[1], &iv[1], &iv[1], l, &c__1, n, liv, 
	    lv, n, &m, p, &v[1], &y[1]);
    if (iv[1] != 14) {
	goto L999;
    }
...
L100:
    rnsg_(&v[a1], &alf[1], &c__[1], &v[da1], &iv[in1], &iv[1], l, &l1, n, liv,
	     lv, n, &m, p, &v[1], &y[1]);
    if ((i__1 = iv[1] - 2) < 0) {
	goto L110;
    } else if (i__1 == 0) {
