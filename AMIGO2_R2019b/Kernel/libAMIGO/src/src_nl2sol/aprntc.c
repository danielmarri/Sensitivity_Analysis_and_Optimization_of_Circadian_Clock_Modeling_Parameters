/* aprntc.f -- translated by f2c (version 20100827).
...
/* Subroutine */ int aprntc_(complex *a, integer *nitems, integer *iout, 
	integer *mcol, integer *w, integer *d__)
{
    ...
    /* Builtin functions */
    double r_lg10(real *), r_imag(complex *);
    integer s_wsfe(cilist *), do_fio(integer *, char *, ftnlen), e_wsfe(void);

    /* Local variables */
    static integer i__, j, k, dd, ww;
    static logical dup;
    static complex line[18];
    static integer emin, emax, ncol;
    static complex last[18];
...
/*  THIS SUBROUTINE PRINTS OUT NITEMS FROM THE COMPLEX ARRAY, A, ON */
/*  OUTPUT UNIT IOUT, USING A MAXIMUM OF MCOL PRINT SPACES. */
/*  THE OUTPUT FORMAT IS 2(1PEW.D). */
...
/*    A        - THE START OF THE COMPLEX ARRAY TO BE PRINTED */
...
/* /6S */
/*     IF (NITEMS .LE. 0) CALL */
/*    1  SETERR(27H  APRNTC - NITEMS .LE. ZERO, 27, 1, 2) */
/* /7S */
    if (*nitems <= 0) {
	seterr_("  APRNTC - NITEMS .LE. ZERO", &c__27, &c__1, &c__2, (ftnlen)
		27);
    }
/* / */
...
/* /6S */
/*     IF (W .GT. MCOL) CALL */
/*    1  SETERR(22H  APRNTC - W .GT. MCOL, 22, 2, 2) */
/* /7S */
    if (*w > *mcol) {
	seterr_("  APRNTC - W .GT. MCOL", &c__22, &c__2, &c__2, (ftnlen)22);
    }
/* / */

/* /6S */
/*     IF (D .LT. 0) CALL */
/*    1  SETERR(22H  APRNTC - D .LT. ZERO, 22, 3, 2) */
/* /7S */
    if (*d__ < 0) {
	seterr_("  APRNTC - D .LT. ZERO", &c__22, &c__3, &c__2, (ftnlen)22);
    }
/* / */

/* /6S */
/*     IF (W .LT. D+6) CALL */
/*    1  SETERR(21H  APRNTC - W .LT. D+6, 21, 4, 2) */
/* /7S */
    if (*w < *d__ + 6) {
	seterr_("  APRNTC - W .LT. D+6", &c__21, &c__4, &c__2, (ftnlen)21);
    }
/* / */
...
/* Computing MAX */
/* Computing MIN */
    i__3 = 9, i__4 = (min(*mcol,160) - indw) / (ww << 1);
    i__1 = 1, i__2 = min(i__3,i__4);
    ncol = max(i__1,i__2);
    i__1 = ncol << 1;
    s88fmt_(&c__1, &i__1, ifmt2 + 10, (ftnlen)1);
    ww += -2;
...
/* /C */
    i__1 = j - 1;
    i__2 = i__;
    line[i__1].r = a[i__2].r, line[i__1].i = a[i__2].i;
/* / */
    if (j == 1) {
	iline = i__;
    }
    ...
    for (k = 1; k <= i__1; ++k) {
/* /R */
/*             IF (LAST(1,K) .NE. LINE(1,K)  .OR. */
/*    1            LAST(2,K) .NE. LINE(2,K)) */
/*    2            DUP = .FALSE. */
/* /C */
	i__2 = k - 1;
	i__3 = k - 1;
	if (last[i__2].r != line[i__3].r || r_imag(&last[k - 1]) != r_imag(&
		line[k - 1])) {
	    dup = FALSE_;
	}
/* / */
/* L30: */
    ...
    goto L70;
/* /R */
/*  40         WRITE(IOUT, IFMT2C) BLANK, ILAST, (LAST(1,K), */
/*    1              LAST(2,K), K=1,NCOL) */
/*  50     WRITE(IOUT, IFMT2C) BLANK, ILINE, (LINE(1,K), */
/*    1              LINE(2,K), K=1,J) */
/* /C */
L40:
    io___24.ciunit = *iout;
    ...
    i__1 = ncol;
    for (k = 1; k <= i__1; ++k) {
	do_fio(&c__2, (char *)&last[k - 1], (ftnlen)sizeof(real));
    }
    e_wsfe();
L50:
    io___26.ciunit = *iout;
    ...
    i__1 = j;
    for (k = 1; k <= i__1; ++k) {
	do_fio(&c__2, (char *)&line[k - 1], (ftnlen)sizeof(real));
    }
    e_wsfe();
/* / */
    count = 1;
...
/* /R */
/*           LAST(1,K) = LINE(1,K) */
/*  60       LAST(2,K) = LINE(2,K) */
/* /C */
/* L60: */
	i__2 = k - 1;
	i__3 = k - 1;
	last[i__2].r = line[i__3].r, last[i__2].i = line[i__3].i;
    }
/* / */
L70:
    ilast = iline;
