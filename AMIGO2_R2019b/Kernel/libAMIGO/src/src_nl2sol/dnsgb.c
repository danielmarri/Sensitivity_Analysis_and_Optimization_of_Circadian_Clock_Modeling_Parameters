/* dnsgb.f -- translated by f2c (version 20100827).
...
/* Subroutine */ int dnsgb_(integer *n, integer *p, integer *l, doublereal *
	alf, doublereal *b, doublereal *c__, doublereal *y, S_fp calca, S_fp 
	calcb, integer *inc, integer *iinc, integer *iv, integer *liv, 
	...
	ufparm)
{
    ...
    extern /* Subroutine */ int drnsgb_(doublereal *, doublereal *, 
	    doublereal *, doublereal *, doublereal *, integer *, integer *, 
	    integer *, integer *, integer *, integer *, integer *, integer *, 
	    integer *, integer *, doublereal *, doublereal *), divset_(
...
/*     DOUBLE PRECISION ALF(P), B(2,P), C(L), URPARM(1), V(LV), Y(N) */
/* /7 */
/* / */
...
/* T(1)...T(N),  DNSGB ATTEMPTS TO COMPUTE A LEAST SQUARES FIT */
...
/*                      CALL DIVSET(1,IV,LIV,LV,V) AND THEN ASSIGNING */
/*                      NONDEFAULT VALUES TO THE APPROPRIATE COMPONENTS */
/*                      OF IV AND V BEFORE CALLING  DNSGB. */
...
/* DRNSGB... CARRIES OUT NL2SOL ALGORITHM. */
...
L60:
    drnsgb_(&v[1], &alf[1], &b[3], &c__[1], &v[1], &iv[1], &iv[1], l, &c__1, 
...
L100:
    drnsgb_(&v[a1], &alf[1], &b[3], &c__[1], &v[da1], &iv[in1], &iv[1], l, &
	    l1, n, liv, lv, n, &m, p, &v[1], &y[1]);
    if ((i__1 = iv[1] - 2) < 0) {
	goto L110;
    } else if (i__1 == 0) {
	goto L120;
