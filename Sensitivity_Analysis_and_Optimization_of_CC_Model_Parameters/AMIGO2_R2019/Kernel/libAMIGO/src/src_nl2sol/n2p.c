/* n2p.f -- translated by f2c (version 20100827).
...
/* Subroutine */ int n2p_(integer *n, integer *nd, integer *p, real *x, S_fp 
	calcr, S_fp calcj, integer *iv, integer *liv, integer *lv, real *v, 
	integer *ui, real *ur, U_fp uf)
{
...
/*  ***  VERSION OF NL2SOL THAT CALLS   RN2G AND HAS EXPANDED CALLING */
...
/* ---------------------------------  BODY  ------------------------------ */
    ...
    rn2g_(&v[1], &v[1], &iv[1], liv, lv, n, &nd1, &n1, &n2, p, &v[1], &v[1], &
	    v[1], &x[1]);
    if (iv[1] != 14) {
	goto L999;
    }
...
L20:
    rn2g_(&v[d1], &v[dr1], &iv[1], liv, lv, n, &nd1, &n1, &n2, p, &v[r1], &v[
	    rd1], &v[1], &x[1]);
    iv1 = iv[1];
    if ((i__1 = iv1 - 2) < 0) {
	goto L40;
