/* d7dog.f -- translated by f2c (version 20100827).
...
/* Subroutine */ int d7dog_(real *dig, integer *lv, integer *n, real *nwtstp, 
	real *step, real *v)
{
    ...
    /* Local variables */
    static integer i__;
    static real t, t1, t2, cfact, relax, cnorm, gnorm, rlambd, ghinvg, femnsq,
	     ctrnwt, nwtnrm;
...
/*     ***  CAUCHY STEP = -CFACT * G. */
    cnorm = gnorm * cfact;
    relax = 1.f - v[43] * (1.f - gnorm * cnorm / ghinvg);
    if (rlambd < relax) {
	goto L50;
    }
...
/*        ***  STEP IS BETWEEN RELAXED NEWTON AND FULL NEWTON STEPS  *** */
    ...
    v[5] = 1.f - (rlambd - relax) / (1.f - relax);
    t = -rlambd;
    v[4] = t * ghinvg;
    v[7] = rlambd * (1.f - rlambd * .5f) * ghinvg;
    v[46] = t;
    i__1 = *n;
    for (i__ = 1; i__ <= i__1; ++i__) {
/* L40: */
...
/* Computing 2nd power */
    r__1 = v[44] / gnorm;
    v[7] = v[8] * (gnorm - v[8] * .5f * (r__1 * r__1));
    i__1 = *n;
    for (i__ = 1; i__ <= i__1; ++i__) {
/* L60: */
	step[i__] = t * dig[i__];
...
/* Computing 2nd power */
    r__1 = gnorm;
    v[4] = t1 * (r__1 * r__1) + t2 * ghinvg;
/* Computing 2nd power */
    r__1 = v[44] * t1;
    v[7] = -t1 * gnorm * ((t2 + 1.f) * gnorm) - t2 * (t2 * .5f + 1.f) * 
	    ghinvg - r__1 * r__1 * .5f;
    i__1 = *n;
    for (i__ = 1; i__ <= i__1; ++i__) {
/* L80: */
