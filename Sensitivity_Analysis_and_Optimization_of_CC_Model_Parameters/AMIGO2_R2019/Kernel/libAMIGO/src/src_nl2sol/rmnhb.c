/* rmnhb.f -- translated by f2c (version 20100827).
...
/* Subroutine */ int rmnhb_(real *b, real *d__, real *fx, real *g, real *h__, 
	integer *iv, integer *lh, integer *liv, integer *lv, integer *n, real 
	*v, real *x)
{
    ...
    static real xi;
    static integer dg1, td1, tg1, ipi, ipn, nn1o2;
    ...
    extern /* Subroutine */ int g7qsb_(real *, real *, real *, real *, 
	    ...
	    real *, real *, real *);
    static integer ipiv2, step0, step1;
    ...
    extern /* Subroutine */ int s7ipr_(integer *, integer *, real *), v7ipr_(
	    integer *, integer *, real *), s7lvm_(integer *, real *, real *, 
	    real *), v2axy_(integer *, real *, real *, real *, real *), 
	    v7cpy_(integer *, real *, real *), v7vmp_(integer *, real *, real 
	    *, real *, integer *), parck_(integer *, real *, integer *, 
    ...
    extern /* Subroutine */ int i7pnvr_(integer *, integer *, integer *);
    static integer lstgst, rstrst;


/*  ***  CARRY OUT   MNHB (SIMPLY BOUNDED MINIMIZATION) ITERATIONS, */
...
/*     ONES TO   MNHB (WHICH SEE), EXCEPT THAT V CAN BE SHORTER (SINCE */
/*     THE PART OF V THAT   MNHB USES FOR STORING G AND H IS NOT NEEDED). */
/*     MOREOVER, COMPARED WITH   MNHB, IV(1) MAY HAVE THE TWO ADDITIONAL */
/*     OUTPUT VALUES 1 AND 2, WHICH ARE EXPLAINED BELOW, AS IS THE USE */
/*     OF IV(TOOBIG) AND IV(NFGCAL).  THE VALUE IV(G), WHICH IS AN */
/*     OUTPUT VALUE FROM   MNHB, IS NOT REFERENCED BY  RMNHB OR THE */
/*     SUBROUTINES IT CALLS. */

/* IV(1) = 1 MEANS THE CALLER SHOULD SET FX TO F(X), THE FUNCTION VALUE */
/*             AT X, AND CALL  RMNHB AGAIN, HAVING CHANGED NONE OF THE */
...
/*              RMNHB TO IGNORE FX AND TRY A SMALLER STEP.  THE PARA- */
...
/*                  THE PARAMETER NF THAT   MNHB PASSES TO CALCG IS */
/*             IV(NFGCAL) = IV(7).  IF G(X) AND H(X) CANNOT BE EVALUATED, */
/*             THEN THE CALLER MAY SET IV(NFGCAL) TO 0, IN WHICH CASE */
/*              RMNHB WILL RETURN WITH IV(1) = 65. */
/*                  NOTE --  RMNHB OVERWRITES H WITH THE LOWER TRIANGLE */
...
/*  G7QSB... COMPUTES APPROXIMATE OPTIMAL BOUNDED STEP. */
...
/*  V7IPR... APPLIES PERMUTATION TO VECTOR. */
/*  V7SCP... SETS ALL ELEMENTS OF A VECTOR TO A SCALAR. */
/*  V7VMP... MULTIPLIES (OR DIVIDES) TWO VECTORS COMPONENTWISE. */
...
/*     *** INVERT OLD PERMUTATION ARRAY *** */
	...
	xi = x[i__];
	gi = g[i__];
	if (xi <= b[(i__ << 1) + 1] && gi > 0.f) {
	    goto L120;
	}
	if (xi >= b[(i__ << 1) + 2] && gi < 0.f) {
	    goto L120;
	}
	iv[ipi] = i__;
	++ipi;
...
/*  ***  PERMUTE SCALED GRADIENT AND HESSIAN ACCORDINGLY  *** */
    ...
    ipi = iv[58];
    s7ipr_(n, &iv[ipi], &h__[1]);
    v7ipr_(n, &iv[ipi], &v[dg1]);
    v[1] = 0.f;
    if (iv[48] > 0) {
	v[1] = v2nrm_(&iv[48], &v[dg1]);
    }
...
L220:
    ...
    x01 = iv[43];
    x11 = x01 + *n;
    g7qsb_(&b[3], &d__[1], &h__[1], &g[1], &iv[ipi], &iv[ipn], &iv[ipiv2], &
...
L240:
    v2axy_(n, &x[1], &c_b38, &v[step1], &v[x01]);
    ++iv[6];
    iv[1] = 1;
    goto L450;
...
L290:
    ...
    v2axy_(n, &v[step1], &c_b43, &v[x01], &x[1]);
    v[17] = rldst_(n, &d__[1], &x[1], &v[x01]);
    iv[9] = rstrst;
