/* n2g.f -- translated by f2c (version 20100827).
...
/* Subroutine */ int n2g_(integer *n, integer *p, real *x, S_fp calcr, S_fp 
	calcj, integer *iv, integer *liv, integer *lv, real *v, integer *ui, 
	real *ur, U_fp uf)
{
    ...
    extern /* Subroutine */ int rn2g_(real *, real *, integer *, integer *, 
	    ...
	    integer *, integer *, real *, real *), ivset_(integer *, integer *
	    , integer *, integer *, real *);


/*  ***  VERSION OF NL2SOL THAT CALLS   RN2G  *** */
...
/*     THEN    N2G RETURNS WITH IV(1) = 15.  WHEN    N2G RETURNS, THE */
...
/*     THEN    N2G RETURNS WITH IV(1) = 16.  WHEN    N2G RETURNS, THE */
...
/*        DEFAULT VALUES ARE PROVIDED BY SUBROUTINE IVSET, RATHER THAN */
/*     DFAULT.  THE CALLING SEQUENCE IS... */
/*             CALL IVSET(1, IV, LIV, LV, V) */
/*     THE FIRST PARAMETER IS AN INTEGER 1.  IF LIV AND LV ARE LARGE */
/*     ENOUGH FOR IVSET, THEN IVSET SETS IV(1) TO 12.  OTHERWISE IT */
/*     SETS IV(1) TO 15 OR 16.  CALLING    N2G WITH IV(1) = 0 CAUSES ALL */
/*     DEFAULT VALUES TO BE USED FOR THE INPUT COMPONENTS OF IV AND V. */
/*        IF YOU FIRST CALL IVSET, THEN SET IV(1) TO 13 AND CALL    N2G, */
...
/*     ARE LARGE ENOUGH.  IF SO, THEN    N2G RETURNS WITH IV(1) = 14. */
/*     WHEN CALLED WITH IV(1) = 14,    N2G ASSUMES THAT STORAGE HAS */
