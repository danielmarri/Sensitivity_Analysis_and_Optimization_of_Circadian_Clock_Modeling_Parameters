/* mnh.f -- translated by f2c (version 20100827).
...
/* Subroutine */ int mnh_(integer *n, real *d__, real *x, S_fp calcf, S_fp 
	calcgh, integer *iv, integer *liv, integer *lv, real *v, integer *
	uiparm, real *urparm, U_fp ufparm)
{
    ...
    extern /* Subroutine */ int rmnh_(real *, real *, real *, real *, integer 
...
/*        THIS ROUTINE IS LIKE   MNG, EXCEPT THAT THE SUBROUTINE PARA- */
/*     METER CALCG OF   MNG (WHICH COMPUTES THE GRADIENT OF THE OBJEC- */
...
/*     AS FOR   MNG, WHILE H IS AN ARRAY OF LENGTH N*(N+1)/2 IN WHICH */
...
/*             INITIALIZED BY CALLING   MNH WITH IV(1) = 13.) */
...
/*  RMNH... REVERSE-COMMUNICATION ROUTINE THAT DOES   MNH ALGORITHM. */
...
L20:
    rmnh_(&d__[1], &f, &v[g1], &v[h1], &iv[1], &lh, liv, lv, n, &v[1], &x[1]);
    if ((i__1 = iv[1] - 2) < 0) {
	goto L30;
    } else if (i__1 == 0) {
	goto L40;
