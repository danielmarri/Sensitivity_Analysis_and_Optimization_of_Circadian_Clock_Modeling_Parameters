/* xtrap.f -- translated by f2c (version 20100827).
...
/* Subroutine */ int xtrap_(real *tm, integer *m, integer *nvar, real *ng, 
...
#define ws ((real *)&cstak_1)
    ...
    extern /* Subroutine */ int a0xtrp_(real *, integer *, integer *, real *, 
	    integer *, logical *, real *, real *, real *, real *, real *, 
	    logical *), seterr_(char *, integer *, integer *, integer *, 
	    ftnlen);
...
/*  SCRATCH SPACE ALLOCATED - MIN(M-1,KMAX) REAL WORDS + */
...
/* /6S */
/*     IF (M.LT.1) CALL SETERR(15H XTRAP - M.LT.1,15,1,2) */
/*     IF (NVAR.LT.1) CALL SETERR(18H XTRAP - NVAR.LT.1,18,2,2) */
/*     IF (NG(1).LT.1.0E0) CALL SETERR(19H XTRAP - NG(1).LT.1,19,3,2) */
/*     IF (KMAX.LT.1) CALL SETERR(18H XTRAP - KMAX.LT.1,18,4,2) */
/* /7S */
    ...
    /* Function Body */
    if (*m < 1) {
	seterr_(" XTRAP - M.LT.1", &c__15, &c__1, &c__2, (ftnlen)15);
    }
    if (*nvar < 1) {
	seterr_(" XTRAP - NVAR.LT.1", &c__18, &c__2, &c__2, (ftnlen)18);
    }
    if (ng[1] < 1.f) {
	seterr_(" XTRAP - NG(1).LT.1", &c__19, &c__3, &c__2, (ftnlen)19);
    }
    if (*kmax < 1) {
	seterr_(" XTRAP - KMAX.LT.1", &c__18, &c__4, &c__2, (ftnlen)18);
    }
/* / */
...
/*    1      (38H XTRAP - NG IS NOT MONOTONE INCREASING,38,5,2) */
/* /7S */
	if (ng[i__ - 1] >= ng[i__]) {
	    seterr_(" XTRAP - NG IS NOT MONOTONE INCREASING", &c__38, &c__5, &
		    c__2, (ftnlen)38);
	}
/* / */
/* L10: */
...
/* Computing MIN */
    ...
    a0xtrp_(&tm[1], m, nvar, &ng[1], kmax, xpoly, &t[t_offset], &error[
