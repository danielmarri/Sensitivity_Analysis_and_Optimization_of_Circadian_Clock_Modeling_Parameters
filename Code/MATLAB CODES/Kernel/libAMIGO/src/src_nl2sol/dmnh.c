/* dmnh.f -- translated by f2c (version 20100827).
...
/* Subroutine */ int dmnh_(integer *n, doublereal *d__, doublereal *x, S_fp 
	...
	doublereal *v, integer *uiparm, doublereal *urparm, U_fp ufparm)
{
    ...
    extern /* Subroutine */ int drmnh_(doublereal *, doublereal *, doublereal 
...
/*        THIS ROUTINE IS LIKE  DMNG, EXCEPT THAT THE SUBROUTINE PARA- */
/*     METER CALCG OF  DMNG (WHICH COMPUTES THE GRADIENT OF THE OBJEC- */
...
/*     AS FOR  DMNG, WHILE H IS AN ARRAY OF LENGTH N*(N+1)/2 IN WHICH */
...
/*             INITIALIZED BY CALLING  DMNH WITH IV(1) = 13.) */
...
/* DRMNH... REVERSE-COMMUNICATION ROUTINE THAT DOES  DMNH ALGORITHM. */
...
L20:
    drmnh_(&d__[1], &f, &v[g1], &v[h1], &iv[1], &lh, liv, lv, n, &v[1], &x[1])
	    ;
    if ((i__1 = iv[1] - 2) < 0) {
	goto L30;
    } else if (i__1 == 0) {
