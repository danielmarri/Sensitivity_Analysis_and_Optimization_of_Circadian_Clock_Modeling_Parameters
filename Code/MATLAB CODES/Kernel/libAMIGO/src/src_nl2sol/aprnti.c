/* aprnti.f -- translated by f2c (version 20100827).
...
/* Subroutine */ int aprnti_(integer *a, integer *nitems, integer *iout, 
	integer *mcol, integer *w)
{
    /* Initialized data */
...
/* /6S */
/*     IF (NITEMS .LE. 0) CALL */
/*    1  SETERR(27H  APRNTI - NITEMS .LE. ZERO, 27, 1, 2) */
/* /7S */
    if (*nitems <= 0) {
	seterr_("  APRNTI - NITEMS .LE. ZERO", &c__27, &c__1, &c__2, (ftnlen)
		27);
    }
/* / */
...
/* /6S */
/*     IF (W .GT. MCOL) CALL */
/*    1  SETERR(22H  APRNTI - W .GT. MCOL, 22, 2, 2) */
/* /7S */
    if (*w > *mcol) {
	seterr_("  APRNTI - W .GT. MCOL", &c__22, &c__2, &c__2, (ftnlen)22);
    }
/* / */
...
/* Computing MIN */
    ...
    ww = min(i__1,i__2);
    s88fmt_(&c__2, &ww, ifmt2 + 11, (ftnlen)1);
/* Computing MAX */
/* Computing MIN */
