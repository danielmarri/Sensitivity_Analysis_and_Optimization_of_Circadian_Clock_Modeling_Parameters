/* dhqr2.f -- translated by f2c (version 20100827).
...
/* Subroutine */ int dhqr2_(integer *nm, integer *n, integer *low, integer *
	 ...
	 integer *ierr)
{
    ...
    extern /* Subroutine */ int cddiv_(doublereal *, doublereal *, doublereal 
	    *);
    static doublereal cmplxc[2], cmplxd[2];
    static logical notlas;
    static doublereal cmplxn[2];



/*  THE ABOVE ARE TO BE USED WITH THE PORT CDDIV */
/*  ROUTINE NEEDED HERE FOR DOUBLE COMPLEX IN */
/*  PLACE OF THE RATIOS USED IN THE ORIGINAL HQR2. */
/*  THEY ARE, RESPECTIVELY, NUMERATOR, DENOMINATOR, AND COMPLEX ANSWER. */
...
/*     THIS ROUTINE IS FROM EISPACK (VERSION DATED AUGUST 1983), WITH */
/*     NAMES CHANGED IN ACCORDANCE WITH PORT CONVENTIONS FOR DOUBLE */
/*     PRECISION, AND WITH PROCEDURE CDIV REPLACED BY THE PORT DOUBLE- */
/*     PRECISION COMPLEX DIVISION ROUTINE, CDDIV. */
...
L720:
	cmplxn[0] = 0.;
	cmplxn[1] = -h__[na + en * h_dim1];
	cmplxd[0] = h__[na + na * h_dim1] - p;
	cmplxd[1] = q;
	cddiv_(cmplxn, cmplxd, cmplxc);
	h__[na + na * h_dim1] = cmplxc[0];
	h__[na + en * h_dim1] = cmplxc[1];
L730:
	h__[en + na * h_dim1] = 0.;
	h__[en + en * h_dim1] = 1.;
	enm2 = na - 1;
...
L770:
	    ...
	    cmplxd[1] = q;
	    cddiv_(cmplxn, cmplxd, cmplxc);
	    h__[i__ + na * h_dim1] = cmplxc[0];
	    h__[i__ + en * h_dim1] = cmplxc[1];
	    goto L790;
...
L784:
	    cmplxn[0] = x * r__ - zz * ra + q * sa;
	    cmplxn[1] = x * s - zz * sa - q * ra;
	    cmplxd[0] = vr;
	    cmplxd[1] = vi;
	    cddiv_(cmplxn, cmplxd, cmplxc);
	    h__[i__ + na * h_dim1] = cmplxc[0];
	    ...
	    goto L790;
L785:
	    cmplxn[0] = -r__ - y * h__[i__ + na * h_dim1];
	    cmplxn[1] = -s - y * h__[i__ + en * h_dim1];
	    cmplxd[0] = zz;
	    cmplxd[1] = q;
	    cddiv_(cmplxn, cmplxd, cmplxc);
	    h__[i__ + 1 + na * h_dim1] = cmplxc[0];
	    h__[i__ + 1 + en * h_dim1] = cmplxc[1];
